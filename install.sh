...
if [ "$MODE" = "global" ]; then
    ...
    log ""
    log "ALTERNATIVE (no env var, works in every terminal): instead of exporting"
    log "OPENCODE_CONFIG_DIR, add these two keys to your GLOBAL opencode config"
    log "($CONFIG_BASE/opencode/opencode.json — opencode also accepts opencode.jsonc;"
    log "create it if absent, or merge them into your existing one):"
    log ""
    log '        "default_agent": "guard/normal",'
    log '        "agent": { "build": { "disable": true }, "plan": { "disable": true } }'
    log ""
    log "opencode always reads that global file, so guard/normal becomes the default and"
    log "build/plan disappear in every terminal — no env var, no shell-profile edit. This"
    log "script does not edit that file for you."
    log ""
    ...
    log ""
    log "FALLBACK (agents only): if you activate NEITHER method above, the three level"
