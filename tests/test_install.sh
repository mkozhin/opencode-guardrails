...
check "global: guard/normal.md landed in agent/guard/ dir" \
    "$([ -f "$AGENT_DEST/guard/normal.md" ] && echo 0 || echo 1)"
check "global: guard/ask.md landed in agent/guard/ dir" \
    "$([ -f "$AGENT_DEST/guard/ask.md" ] && echo 0 || echo 1)"
check "global: guard/trust.md landed in agent/guard/ dir" \
    "$([ -f "$AGENT_DEST/guard/trust.md" ] && echo 0 || echo 1)"
...
n_agents="$(find "$AGENT_DEST/guard" -maxdepth 1 -name '*.md' | wc -l | tr -d ' ')"
...
check "project: agents in ./.opencode/agent/guard/" \
...
check "force: differing file overwritten with ours" \
    "$(cmp -s "$REPO_ROOT/agents/normal.md" "$AGENT_DEST/guard/normal.md" && echo 0 || echo 1)"
