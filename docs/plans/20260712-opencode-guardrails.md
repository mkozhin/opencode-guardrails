...
- **Прочие скрытые файлы → `ask`** (на `read`): `.*`, `*/.*` — это **dot-именованные**
  файлы (`.gitignore`, `.eslintrc`; часто нужны легитимно). **Scope-оговорка (⚠️
  устарело — см. коррекцию Task 1, ниже раздел «Корректировки плана по итогам Task 1»,
  и `test_agents.py`):** первоначально считалось, что `*` НЕ пересекает `/`; на самом
  деле матчер — anchored dotall regex, где `*`→`.*` **пересекает** `/`. Поэтому слой
  `.*` уже ловит и `.git/config`, и `.aws/config` (любой путь с dot-сегментом), а
  секреты внутри скрытых папок (`.aws/credentials`, `.ssh/id_rsa`) дополнительно
  покрыты anchored deny-паттернами. Этот слой условен: `--auto`/`always` могут подавить prompt.
...
> → carve-out. ⚠️ **Коррекция Task 1:** `*` в glob **ПЕРЕСЕКАЕТ** `/` (матчер —
> anchored dotall regex, `*`→`.*`), поэтому один `*.pem` ловит и `app.pem`, и
> `nested/app.pem` — парные root+nested паттерны ИЗБЫТОЧНЫ (см. раздел
> «Корректировки плана по итогам Task 1»). Extension-less имена всё равно якорятся
> парой `name` + `*/name`, чтобы матч был привязан к сегменту пути, а не к подстроке.
...
- `install.sh` копирует `agents/*.md` в `~/.config/opencode/agent/` (или
  `./.opencode/agent/` при `--project`) — ⚠️ **коррекция Task 1: каталог `agent/`
  в ЕДИНСТВЕННОМ числе**, не `agents/` (см. строку 79 и раздел коррекций) — и кладёт
  overlay-файл в известное место;
